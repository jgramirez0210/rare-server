...
def get_single_user(id):
    with sqlite3.connect("./db.sqlite3") as conn:
        ...
        if data is not None:
            ...
            return None
def get_all_users_management(id):
    with sqlite3.connect("./db.sqlite3") as conn:
        conn.row_factory = sqlite3.Row
        db_cursor = conn.cursor()
        ...
        # Check if data is not None before trying to access its items
        if data is not None:
            user_management = SerializedUserManagement(data['username'], data['first_name'], data['last_name'],
                        data['email'])
        
            return user_management.user.__dict__
        else:
            return None
def delete_user(id):
